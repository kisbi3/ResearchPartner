...
| CI harness checks | `.github/workflows/harness-checks.yml` | Runs pytest, manifest, spawn-contract, contract-sync, and evaluator `--fail-on-partial` repo-state checker commands on push and pull request; it does not replace live Claude Code hook firing |
