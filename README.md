...
The harness generates a **Current Run Dashboard** (`<project>/ResearchPartner-runs/.../workflow_map.html`) for the latest active run. The central `docs/workflow_map.html` is NOT generated by default — pass `--central` to `scripts/generate_workflow_map.py` if you really want it. The run dashboard provides a real-time view showing:
...
| Start run | `python scripts\start_research_run.py --name "topic name"` | Creates a dated run packet under the in-project `ResearchPartner-runs\` root (override with `--runs-root`), including `docs\process\live_workflow_diagram.md` |
...
| Generate workflow map | `python scripts\generate_workflow_map.py` | Refreshes the latest run's `workflow_map.html` and `workflow_map.json`. Pass `--central` to also build `docs\workflow_map.html` (off by default) |
