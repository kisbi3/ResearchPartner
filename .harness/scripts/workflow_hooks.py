...
``.harness/scripts/sync_workflow.py`` (the ``/sync-workflow`` skill). There is no
...
IN_FLIGHT_HEADER = (
    "## In-Flight Tasks\n\n"
    "<!-- Auto-updated by .harness/scripts/workflow_hooks.py on every Agent() spawn. -->\n\n"
...
def _scaffold(project: Path) -> str:
    return (
        ...
        "Auto-managed by `.harness/scripts/sync_workflow.py` (everything except the\n"
        ...
        "## Real-Time Event Log\n\n"
        "<!-- Append-only. Auto-updated by .harness/scripts/workflow_hooks.py on every Agent() spawn. -->\n"
    )
