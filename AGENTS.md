...
- **Cross-Tier Write Hook (HARD ENFORCED)**: every `<run>/src/*.py` write must come from a spawned Implementation Agent. The PreToolUse hook `scripts/check_src_write_authorization.py` (registered in `.claude/settings.local.json` on `Write|Edit`) blocks any direct `src/*.py` write that lacks a matching row in `<run>/docs/gates/agent_spawn_log.md` or a freshly touched (≤10 min) spawn log. Bypass with `RESEARCH_HARNESS_BYPASS_SRC_GATE=1` for an explicit one-off waiver.
