...
For substantial research plans, existing-project reviews, reproduction attempts, simulation campaigns, analysis pipelines, figure sets, or manuscript-claim work, organize the work as a lead-agent-directed research group of: **Lead Agent** (this is the main conversation context, *not* a spawned subagent — owns scientific judgment, dialogue with the researcher, and gate approval; holds the nine "professor stances"), **Peer-Review Professor** (spawned subagent, `meeting --scope review` only — single-shot adversarial reviewer), **Graduate Test-Design Agents** (spawned — turn Lead-Agent tasks into testable plans), **Coding Subagents** (spawned by Graduate Students — bounded implementation/validation/audit/figure work, never strengthen claims), and **Cartographer** (*not* spawned — hook-driven workflow recorder via `scripts/workflow_hooks.py` and the `cartographer-update` SKILL).
...
**Orchestration mechanics — agent spawning protocol, 2-tier spawn hierarchy (Lead Agent → Graduate Student → Coding Subagents), when to spawn, parallel-spawn rule, per-tier model recommendations, the four spawn-block templates (Graduate Student / Implementation Agent / Scientific Validator / Cache-Log Auditor), cross-tier prohibitions, Live Linked Research Graph specification, the nine Lead-Agent stances, and the completion-conference rule — are kept in `docs/orchestration_protocol.md` so they do not bloat every subagent's context.** Load that file at the start of any substantial research iteration (the Lead Agent always loads it) or when a Graduate Student must spawn sub-agents. Coding subagents do not need to load it: their spawn block plus their own `skills/<role>/SKILL.md` is sufficient.

Required scientific-loop hooks:

- **Session Resumption Hook**: at the start of any new session that may be continuing prior work, run `python scripts/check_session_resumable.py` before the Task Intake Hook (the script auto-discovers the most recently modified run under the in-project `ResearchPartner-runs/` directory (legacy sibling location is also checked for backward compat); pass `--run <run-dir>` to target a specific run). The script reads the live workflow diagram's `## In-Flight Tasks` table and `## Gate Status` table. If the report lists in-flight sub-agent tasks (`spawned` rows) or blocked/`in_progress` gates, surface each one to the researcher and confirm whether to `continue`, `retry`, or `abandon` before doing any other research action. Record each decision with `python scripts/update_workflow_diagram.py --event resume --step "..." --agent "professor-orchestrator" --task-id <id> --resume-decision continue|retry|abandon`. Before spawning any sub-agent via `Agent()`, log the spawn with `--event spawn --task-id <id> --step "..." --agent "<role>" --evidence-record "..."` so a future interrupted session can find and reconcile the abandoned task.
...
- **Cross-Tier Write Hook (HARD ENFORCED)**: `.py`/`.ipynb` writes inside a run directory go through spawned Implementation Agents only. See [`docs/hooks_reference.md`](docs/hooks_reference.md#cross-tier-write-hook-hard-enforced).
...
- **Cross-Tier Compliance Gate Hook**: `python scripts/check_cross_tier_compliance.py --run <run-dir>` before a stage gate; backstop for the two write hooks. See [`docs/hooks_reference.md`](docs/hooks_reference.md#cross-tier-compliance-gate-hook).
- **Spawn Log Integrity Hook**: `python scripts/check_spawn_log_integrity.py --run <run-dir>` reconciles `agent_spawn_log.md` rows against Agent() events recorded in the live diagram, catching forged rows. See [`docs/hooks_reference.md`](docs/hooks_reference.md#spawn-log-integrity-hook).
- **Claim Promotion Gate Hook (HARD ENFORCED)**: `python scripts/check_claim_promotion.py --run <run-dir> --target <ceiling>` enforces both a count gate and a diversity gate (mechanism needs a baseline-class pass; generalization needs ≥2 distinct check categories). See [`docs/hooks_reference.md`](docs/hooks_reference.md#claim-promotion-gate-hook-hard-enforced).
...
- **Cartographer Artifact Hook**: whenever the active step, gate status, evidence link, waiver, blocked behavior, stale artifact, or next review checkpoint changes, call `python scripts/update_live_json.py --run <run-dir> [--active-step "..."] [--gate "..." --status <status> --note "..."] [--event '<json>']`. This writes directly to the run-local `workflow_map.live.json` that `workflow_map.html` polls every 10 seconds — no `generate_workflow_map.py` run required. Also update the Gate Status table and Active Step section in `docs/process/live_workflow_diagram.md` for the human-readable research log. `scripts/workflow_hooks.py` auto-appends events to the live diagram on Agent spawns and on Write/Edit of signal artifacts (phase notes, gate logs, stage checkpoints, meeting records, figures, errors, cache, model versions under `docs/model_versions/`, paper reviews under `literature/reviews/`, and claims under `docs/claims/`). For the lineage-bearing signal artifacts (`docs/model_versions/<id>.md`, `literature/reviews/<paper_id>.md`, `docs/claims/<id>.md`, `outputs/figures/*`, `errors/*.err`), the hook **also seeds a typed lineage node** in `workflow_map.live.json` (with `lineage_kind`, `node_type`, and the identifying field such as `paper_id`, `model_version`, or `thumbnail_path` derived from the path). Skills only need to emit cross-referential edges (`evolved_from`, `reproduces`, `cites_paper`, `supports`, `limits`) on top of those auto-seeded nodes — see the Worked Examples section in `skills/cartographer-update/SKILL.md`. See [`docs/hooks_reference.md`](docs/hooks_reference.md#cartographer-artifact-hook).
- **Lineage Graph Hook**: `workflow_map.html` ships with a Lucidchart-style **Lineage tab** (Cytoscape.js + dagre, vendored under `docs/vendor/`) that renders Papers → Decisions → Model Versions → Results → Claims for the current run, plus an optional **Cross-Run Lineage** tab built by `python scripts/build_lineage_graph.py` (writes `ResearchPartner-runs/_index/lineage_graph.json`). When a Cartographer packet adds a new paper review, model version, figure, claim, anomaly, or waiver, set `lineage_kind` (`paper | decision | model_version | result | claim | figure | anomaly | waiver | gate`) and any of `model_version`, `paper_id`, `thumbnail_path`, `parent_run` so the new node renders with the correct shape and connects to its ancestry. Use `graph_links.relation` values `evolved_from` (model version chain), `reproduces` (target paper or prior run baseline), and `cites_paper` (decision or claim citing a paper) to draw cross-step lineage edges. Re-run `python scripts/build_lineage_graph.py` after a run boundary or after `parent_run` references change. To see what a fully populated lineage looks like (papers + decisions + model versions + results + figures + anomalies + claims with the correct edge relations), run `python scripts/seed_lineage_demo.py` then `python scripts/build_lineage_graph.py`; this writes a synthetic `ResearchPartner-runs/_demo_lineage/` run that appears alongside real runs and is clearly labeled as illustrative.
- **Lineage Coverage Gate**: `python scripts/check_lineage_coverage.py --run <run-dir> [--strict]` flags claim nodes without supports edges, non-initial model_versions without evolved_from edges, orphan paper reviews (no incoming cites_paper/reproduces), and unresolved anomalies without limits edges. Surfaces silent failures where a skill's `Cartographer Update` step was skipped. The Stage Checkpoint embeds the violation table; promote `--strict` before claim-ceiling escalation. See [`docs/hooks_reference.md`](docs/hooks_reference.md#lineage-coverage-gate).
- **Broken-Edge Linter**: `python scripts/update_live_json.py --run <run-dir> --validate` checks every `graph_links` and `edges` reference resolves to a real node id in the same map. Catches typos that Cytoscape silently drops. See [`docs/hooks_reference.md`](docs/hooks_reference.md#broken-edge-linter).
...
- **Meeting Hook**: when "does this make sense?" cannot be answered reliably alone — especially after unexpected results, before high-stakes claims, or when the research direction may be biased — convene a meeting using the `meeting` skill. Graduate students may call `--scope quick` (Lead Agent only); the Lead Agent may call `--scope review` (adds Peer-Review Professor); the researcher may call any scope. The live workflow diagram is always shared. Record the resolution in `docs/meetings/YYYY-MM-DD-<slug>.md`. If outcome is "documented disagreement", invoke `cartographer-update` to record the open question as a visible workflow node.
...
The Cartographer (hook-driven, not a spawned agent) maintains a **Live Linked Research Graph** (not just a static loop diagram), with Code/Result/Interpretation links, Link Status, Evidence Strength, Claim ceiling, Researcher Checkpoint markers, Artifact Previews, and staleness propagation. Full schema and the Lead Agent's nine stances + completion-conference rule are in `docs/orchestration_protocol.md`.
