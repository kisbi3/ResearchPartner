...
- **Cross-Tier Write Hook (HARD ENFORCED)**: every `.py` or `.ipynb` file written anywhere under a `ResearchPartner-runs/<run>/` directory (except `<run>/docs/` and `<run>/literature/`) must come from a spawned Implementation Agent — the Lead Agent and Graduate Students are forbidden from writing code (Graduate Students *review* code; if a change is needed they re-spawn the Implementation Agent). The PreToolUse hook `scripts/check_src_write_authorization.py` (registered in `.claude/settings.local.json` on `Write|Edit`) blocks any covered write that lacks a matching row in `<run>/docs/gates/agent_spawn_log.md` or a freshly touched (≤10 min) spawn log. Bypass with `RESEARCH_HARNESS_BYPASS_SRC_GATE=1` for an explicit one-off waiver.
- **Cross-Tier Compliance Gate Hook**: before advancing a stage gate, run `python scripts/check_cross_tier_compliance.py --run <run-dir>` (add `--strict` to also fail when no spawn log exists). The script reuses the same counting logic as the stage checkpoint and exits non-zero when `src/*.py` files exist without matching Implementation Agent spawn records.
- **Claim Promotion Gate Hook (HARD ENFORCED)**: before the Lead Agent promotes the run's claim ceiling above `observation`, run `python scripts/check_claim_promotion.py --run <run-dir> --target <observation|interpretation|mechanism|generalization>`. The script reads `<run>/docs/gates/validation_log.md` (Date | Check | Target | Status | Evidence) and refuses promotions whose `pass` count is below the threshold: interpretation ≥ 1, mechanism ≥ 2, generalization ≥ 3. Lower the target ceiling or add a waivered validation row if the evidence is intentionally limited.
- **Peer-Review Invocation Hook (HARD ENFORCED)**: a Peer-Review Professor may only be spawned from within a `meeting --scope review` (or `--scope full`) session. The PreToolUse hook `scripts/check_peer_review_invocation.py` (registered on `Agent`) inspects the spawn prompt for the Peer-Review role and blocks the spawn unless the prompt itself references the `meeting` skill with `--scope review`/`--scope full`, or a `<run>/docs/meetings/*.md` artifact was touched within the last 10 minutes. Bypass with `RESEARCH_HARNESS_BYPASS_MEETING_GATE=1`.
- **Cartographer Artifact Hook**: `scripts/workflow_hooks.py` now also fires on `Write|Edit` (PreToolUse + PostToolUse). When a signal artifact under a run directory is written — `docs/orient_note.md`, `docs/interview_notes.md`, `docs/literature_review_plan.md`, `docs/model_spec.md`, `docs/baseline_strategy.md`, `docs/research_plan.md`, `docs/replanning_memo.md`, `docs/gates/agent_spawn_log.md`, `docs/gates/validation_log.md`, `docs/research_retrospective.md`, `docs/checkpoints/stage_*_checkpoint.md`, or `docs/meetings/*.md` — the live workflow diagram automatically appends an event-log line and (where appropriate) flips the matching gate status. The Lead Agent no longer needs to invoke `cartographer-update` for these well-known artifacts, though explicit packets remain available for non-routine state changes.
- **Re-spawn Monitoring Hook**: `scripts/write_stage_checkpoint.py` reports re-spawn hotspots (files with ≥3 Implementation Agent entries in `docs/gates/agent_spawn_log.md`) alongside the cross-tier verdict. Re-spawns are normal (the Graduate Student code review can reject a draft and re-spawn the Implementation Agent), but a hotspot signals a poor spec, an ambiguous task, or a buggy Implementation Agent pass that the researcher should inspect at the stage gate.
