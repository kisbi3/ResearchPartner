...
- **Session Resumption Hook**: at the start of any new session that may be continuing prior work, run `python scripts/check_session_resumable.py` before the Task Intake Hook (the script auto-discovers the most recently modified run under a sibling `ResearchPartner-runs` directory; pass `--run <run-dir>` to target a specific run). The script reads the live workflow diagram's `## In-Flight Tasks` table and `## Gate Status` table. If the report lists in-flight sub-agent tasks (`spawned` rows) or blocked/`in_progress` gates, surface each one to the researcher and confirm whether to `continue`, `retry`, or `abandon` before doing any other research action. Record each decision with `python scripts/update_workflow_diagram.py --event resume --step "..." --agent "professor-orchestrator" --task-id <id> --resume-decision continue|retry|abandon`. Before spawning any sub-agent via `Agent()`, log the spawn with `--event spawn --task-id <id> --step "..." --agent "<role>" --evidence-record "..."` so a future interrupted session can find and reconcile the abandoned task.
