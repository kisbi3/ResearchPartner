...
``literature/`` (PDFs and review notes), and ``scripts/`` / ``tools/`` (vendored
...
# docs/ + literature/ hold notes and PDFs; scripts/ + tools/ are vendored harness
...
EXEMPT_SUBDIRS = {"docs", "literature", "scripts", "tools"}
...
def main() -> int:
    ...
    print(
        ...
        f"        the project (docs/, literature/, scripts/, tools/ are exempt).\n"
