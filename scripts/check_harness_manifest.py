...
MIN_CAPABILITIES = 8
PROJECT_DIR_TOKEN = "$CLAUDE_PROJECT_DIR/scripts/"
...
def _workflow_gate_keys(project: Path) -> set[str]:
    source = project / "scripts" / "generate_workflow_map.py"
...
def _command_script(command: str) -> str:
    token_match = re.search(r"\$CLAUDE_PROJECT_DIR/scripts/([^\"\s]+)", command)
    if token_match:
        return f"scripts/{token_match.group(1)}"
    relative_match = re.search(r"\bscripts/([^\"\s]+)", command)
    if relative_match:
        return f"scripts/{relative_match.group(1)}"
    return ""
